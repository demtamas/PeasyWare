...
/// This class is responsible for creating connections, executing stored procedures,
/// and mapping database results to C# model objects.
/// </summary>
public class DatabaseService
{
    /// <summary>
    /// Stores the database connection string loaded from appsettings.json.
    /// It is read-only to ensure it's not changed after initialization.
    /// </summary>
    private readonly string _connectionString = string.Empty;

    /// <summary>
    /// Initializes a new instance of the DatabaseService class.
    /// It reads the database connection string from the appsettings.json file.
    /// </summary>
    /// <exception cref="InvalidOperationException">Thrown if the connection string is not found in the configuration file.</exception>
    public DatabaseService()
    {
        ...
        _connectionString = configuration.GetConnectionString("DefaultConnection")?.ToString() ?? string.Empty;
    ...
    public async Task<SessionConfig> LoadConfigurationAsync()
    {
        ...
        {
            Console.WriteLine($"FATAL: Could not connect to database or load configuration. {ex.Message}");
            Environment.Exit(1);
        }
        return config;
    }
